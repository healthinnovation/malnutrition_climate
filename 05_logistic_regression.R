...
# 3. Entrenar el modelo -> Logistic Regression .................................

train_and_evaluate_logistic_regression_models <- function(option) {
  results <- list()
  ...
  for (eval_year in 15:19) {
    ...
    numeric_vars <- setdiff(names(training_set), "malnutrition")
    training_set[numeric_vars] <- scale(training_set[numeric_vars])
    test_set[numeric_vars] <- scale(test_set[numeric_vars])
    
    # Entrenar el modelo de Regresión Logística
    lr_model <- glm(malnutrition ~ ., data = training_set, family = binomial)
    
    # Realizar predicciones en el conjunto de prueba
    predictions <- predict(lr_model, test_set, type = 'response')
    
    # Convertir probabilidades en clases (usando un umbral de 0.5)
    predictions_yes <- ifelse(predictions >= 0.5, 1, 0)
