...
malnutrition_14 <- read_csv(path_2014) %>%
  mutate(HHID = as.character(HHID), year = 2014)
...
datos <- data.frame(rbind(malnutrition_14, malnutrition_17)) %>%
...
datos %>%
  gather(key = "variable", value = "valor", -year) %>%
  ggplot(aes(x = valor, fill = year)) +
