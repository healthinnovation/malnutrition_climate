...
sets <- create_training_and_test_sets(14:15, 16, c(opt_1,opt_5,opt_6))
...
area1 <- st_as_sf(d1, coords = c("longitudx", "latitudy"))
area2 <- st_as_sf(d2, coords = c("longitudx", "latitudy"))
...
rf_testing_pred <- 
  predict(rf_fit, test_set, type = "prob") %>%
  mutate(.pred_class =  as.factor(ifelse(.pred_1 > 0.15, "1", "0"))) %>%
  bind_cols(test_set %>% select(malnutrition))
...
```{r}
# Crear conjuntos de entrenamiento y prueba
sets <- create_training_and_test_sets(14:16, 17, c(opt_1,opt_5,opt_6))
...
```{r}
# Suponiendo que rf_region_2019 tiene una columna 'region'
